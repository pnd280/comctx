...
void (async () => {
  ...
  const counter = injectCounter(new InjectAdapter(new URL('./web-worker/index.ts', import.meta.url)))
  ...
  document.querySelector<HTMLDivElement>('#app')!.appendChild(
    createElement(`
      <div>
        ...
        <div class="card">
          <h4 id="background-value">WebWorker Value: ${initValue} </h4>
        </div>
      </div>
    `)
  )
  ...
  counter.onChange((value) => {
    ...
    document.querySelector<HTMLDivElement>('#background-value')!.textContent = `WebWorker Value: ${value}`
  })
})().catch(console.error)
